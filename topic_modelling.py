...
def keywords_to_topic(df):
    """assign a description ('theme') to each topic based on keywords"""
    for ix, row in df.iterrows():
        keys = row['keywords']
        # print(keys)
        if keys == ', '.join(['lack', 'vaccine', 'maybe', 'inject', 'missed']):
            theme = "People are lacking / missing vaccine(s)"
        elif keys == ', '.join(['people', 'get', 'effects', 'know', 'countries']):
            theme = "Vaccine is ineffective or even dangerous"
        elif keys == ', '.join(['dose', 'million', 'pfizer', 'astrazeneca', 'countries']):
            theme = "News on COVID-19 vaccine rollout"
        elif keys == ', '.join(['because', 'fda', 'pfizer', 'emergency', 'approve']):
            theme = "COVID-19 vaccine approval process"
        elif keys == ', '.join(['gave', 'senate', 'price', 'detente', 'president']):
            theme = "Government and/or others are profiting from the situation"
        elif keys == ', '.join(['citizen', 'gave', 'government', 'vaccine', 'mayor']):
            theme = "Opinions of politicians over COVID-19 vaccine rollout"
        else:
            theme = "Unclear / unknown"
        df.at[ix, "topic"] = theme
    return df


@click.command()
@click.option('--refit', default=False, is_flag=True)
@click.option('--modelname', default='gsdmm_model')
def main(refit, modelname):
    # load data
    df_tweets = pd.read_csv('tweets/tweets_latest_sentiment_select.csv')
    ...
    if not refit:
        if os.path.exists('gsdmm_model.pickle'):
            ...
            model = pickle.load(open(f"{modelname}.pickle", "rb"))
        else:
            print('no model found, break')
            return 0
    else:
        ...
        pickle.dump(model, open(f"{modelname}.pickle", "wb"))
    ...
    print('create list of human-readable topics (de-lemmatize)')
    ...
    # print(topic_list_human_readable)
    ...
    df = keywords_to_topic(df)
    print(df.topic.unique())

    # assign topic to tweets
    print('assign topic to tweets')
    ...
    df_tweets.to_csv('tweets/tweets_latest_sentiment_topic_select.csv', index=False)

    df.to_csv('topics/topics_latest_select.csv')
